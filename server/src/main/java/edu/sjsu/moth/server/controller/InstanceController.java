...
public class InstanceController {
    ...
    private static final ConfigurationV1 CONFIGURATIONV1 = new ConfigurationV1(STATUSES, MEDIA_ATTACHEMENTSV1,
                                                                               POLLS);
    private static final Accounts ACCOUNTS = new Accounts(10);
    private static final Translation TRANSLATION = new Translation(true);
    private static final ConfigurationV2 CONFIGURATIONV2 = new ConfigurationV2(URLS, ACCOUNTS, STATUSES, MEDIA_ATTACHEMENTSV1,
                                                                               POLLS, TRANSLATION);
    ...
    public Mono<ResponseEntity<InstanceV2>> getV2Instance() {
        return getContactAccountV2().map(c -> ResponseEntity.ok(new InstanceV2(
                ...
                new String[]{"en"},
                CONFIGURATIONV2,
                new RegistrationsV2(false, false, null),
                new Contact("", c),
                RULES
        )));
    }
    ...
    public record InstanceV1(
            ...
            ConfigurationV1 configuration,
            AccountV1 contact_account,
            Rule[] rules
    ) {}

    public record InstanceV2(
            String domain,
            String title,
            String version,
            String sourceUrl,
    ...
    public record MediaAttachments(
            String[] supported_mime_types,
            int image_size_limit,
            int image_matrix_limit,
            int video_size_limit,
            int video_frame_limit,
            int video_matrix_limit
    ) {}

    public record Polls(
            int max_options,
            int max_characters_per_option,
            int min_expiration,
            int max_expiration
    ) {}
